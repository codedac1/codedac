...
└── scripts/
    ...
    ├── page_lastmod.json     # 페이지별 해시+변경일 캐시 — gen_site.js 가 갱신, 커밋 필요
    ...
    ├── gen_site.js           # ★ 사이트 생성기 (252개 페이지 + 껍데기 + sitemap 전부 생성)
...
- **스토어 지표** — `scripts/store_stats.json`: Google Play에서 수집한 앱별 평점·설치수 + 집계. 홈 히어로의 지표 스트립(출시 앱 수 · 누적 다운로드 · 지원 언어 수)에 쓰이며, 이 파일이 없으면 지표는 자동 생략됩니다. 집계 평점은 **화면에 표시하지 않으므로** 앱 상세의 `SoftwareApplication` 구조화 데이터에도 `aggregateRating`을 넣지 않습니다 — 구글은 페이지에 보이지 않는 평점 마크업을 금지합니다.
- **페이지별 변경일** — `scripts/page_lastmod.json`: 렌더된 페이지의 해시와 마지막 변경일. 생성기가 자동으로 갱신하며 **커밋해야 합니다**. sitemap의 `<lastmod>`가 여기서 나옵니다(아래 참고).
...
```bash
node scripts/gen_site.js     # 21개 언어 × (홈 + 앱상세 10 + 개인정보처리방침) = 252개 페이지 + sitemap.xml 생성
```
...
- **`<lastmod>`** — 페이지마다 **실제로 바뀐 날**을 넣습니다. 렌더된 HTML 의 해시를 `scripts/page_lastmod.json` 에 캐시해 두고, 해시가 달라진 페이지만 날짜를 올립니다(자산 캐시 버전 `?v=NN` 은 본문 변경이 아니므로 무시). 구글은 lastmod 가 일관되게 정확할 때만 사용하고, 매 빌드마다 전 URL 을 오늘로 찍으면 아예 믿지 않게 됩니다.
...
3. `scripts/gen_site.js`의 `LANGS` 배열에 `{ code, hreflang, htmlLang, ogLocale, native }` 한 줄 추가(RTL 이면 `dir: 'rtl'`).
